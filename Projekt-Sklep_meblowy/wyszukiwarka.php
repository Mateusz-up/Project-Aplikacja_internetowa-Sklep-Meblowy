...
<div class="pasreje">
  ...
  ?>
	...
	function wyszukiwarka($dane)

{
	
	global $link;
	
	$wyniki = mysqli_query($link,"Select * from produkty where nazwa LIKE '%$dane%' ") 
...
echo'</div>';
	...
	$wyniki2 = mysqli_query($link,"Select * from promocja where nazwa LIKE '%$dane%' ") 
or die('Błąd zapytania'); 

	if(mysqli_num_rows($wyniki2) > 0)
	{
	while($rr = mysqli_fetch_object($wyniki2))
	{
		
		$id=$rr->id;
		$idik=$rr->id_opisu_zdj;
       echo '<div class="element">';
		echo '<div class="obramowanie">';
		echo"<h10>";
	   echo "<a style='text-decoration:none'  target='_blank' href='product2.php?product_id=$id'>";
        echo "<b>".$rr->nazwa."</b>"; 
		
	echo "<br> </br>";		
      
  
		 echo" <div class='tooltip2'> <img class='zoom' src=images/".$rr->id_zdjec.".jpg width='190' height'120'>";
		 //echo"<span class='tooltiptext2'> $idik</span>";
...
$wyniki2 = mysqli_query($link,"Select * from promocja where nazwa LIKE '%$dane%' ") 
or die('Błąd zapytania'); 
	if(mysqli_num_rows($wyniki) > 0 || (mysqli_num_rows($wyniki2) > 0))
	{
	}
	else
	{
		
		echo '<div class="wyszukiobwzle">';
		echo"<h655>";
		echo"Brak pasujących produktów ";
		echo"</h655>";
	echo"</div>";
	}
}
	...
	echo '<div class="wyszukiobw">';
	echo"<h25>";
	echo"Wynik wyszukiwania dla: $dane";
	echo"</h25>";
	echo"</div>";
	echo"<br><br><br>";
	wyszukiwarka($dane);
